...
const PROPERTY_TYPES = ["All", "House", "Apartment", "Villa", "Hotel", "Warehouse"];
...
export default function Hero() {
  ...
  // 🎯 FIXED PIPELINE: Routes query parameters directly onto your custom Search Engine view
  const executeHeroSearchSubmit = (e) => {
    e.preventDefault();
    const parameters = new URLSearchParams();
    
    // Inject the active search values into the URL query line parameter nodes
    ...
    // 🎯 REDIRECT PATCH: Shifts navigation direction over to your optimized SearchPage query stream
        ...
        <div className="relative z-10 w-full max-w-[680px] flex flex-col justify-center">
          ...
          <div className="relative w-full max-w-[655px]">
            ...
            >
              ...
              {/* 🏢 FIELD 1: STANDALONE WHITE CARD BLOCK - LOCATION */}
              <div 
                  ...
                  <div className="absolute left-0 right-0 top-13 bg-white border border-slate-200 rounded-lg shadow-xl max-h-40 overflow-y-auto z-50 py-1 text-xs text-slate-700">
                    {filteredLocations.map((loc, i) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 hover:bg-slate-50 cursor-pointer border-0 bg-transparent text-slate-700 font-bold"
                      >
                        📍 {loc}
                      </button>
                    ))}
              ...
              {/* 🏢 FIELD 2: STANDALONE WHITE CARD BLOCK - PROPERTY TYPE */}
              <div 
                className="flex-1 h-[46px] bg-white rounded-lg px-3 flex items-center justify-between gap-2 shadow-sm border border-slate-100 relative" 
                ref={typeDropdownRef}
              >
                ...
                {showTypeDropdown && (
                  <div className="absolute left-0 right-0 top-[50px] bg-white border border-slate-200 rounded-lg shadow-xl z-50 py-1 text-xs text-slate-700 overflow-hidden font-bold">
                    {PROPERTY_TYPES.map((typeOption) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-2 hover:bg-slate-50 transition-colors border-0 bg-transparent font-bold text-slate-700"
                      >
                        {typeOption}
                      </button>
                    ))}
              ...
              {/* 🏢 FIELD 3: STANDALONE WHITE CARD BLOCK - MAX PRICE */}
              {/* 🎯 FIXED INPUT STATE LINK: Hooked value and onChange events straight to the maxPrice tracking state */}
